...
from app.api.endpoints import auth, playbooks, admin, common, user_favorites, admin_configuration, ansible, collaboration, variable_types, galaxy_roles, playbook_export, galaxy_sources, projects, project_artifacts, project_shares, git_credentials, yaml_parser, git_import, git_operations, git_sync, git_pull_request, collection
...
api_router.include_router(git_pull_request.router)  # Git pull request management
...
api_router.include_router(git_credentials.router)  # Git credentials
api_router.include_router(collection.router)  # Collection/requirements management
api_router.include_router(yaml_parser.router)  # YAML parser (Ansible YAML → graph)
